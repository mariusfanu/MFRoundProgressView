...
class MFRoundProgressView: UIView {
    ...
    var titleColor = UIColor(red: 1, green: 1, blue: 1, alpha: 0.8)
    ...
    override func draw(_ rect: CGRect) {
        ...
        let progressBackgroundPath = UIBezierPath(
            ovalIn:
                CGRect(
                    x: rect.minX + kMFPadding / 2,
                    y: rect.minY + kMFPadding / 2,
                    width: rect.size.width - kMFPadding,
                    height: rect.size.height - kMFPadding
                )
        )
        ...
        progressPath.addArc(
            ...
            endAngle: (endAngle - startAngle) * (percent / 100) + startAngle,
            clockwise: true
        )
